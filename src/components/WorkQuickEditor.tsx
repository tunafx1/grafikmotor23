...
import { AlignCenter, Image as ImageIcon, Move, Ratio, Type, Upload } from 'lucide-react';
  ...
  return <aside className="work-quick-editor" aria-label="Mevcut tasarım ayarları">
    ...
    <div className="work-quick-editor-body">
      ...
      {selected && <section className="work-selected-control">
        ...
        {selected.type === 'text' ? <textarea rows={5} value={textValue} onChange={event => props.onTextChange(selected.id, event.target.value)} placeholder="Başlık veya açıklama metni"/> : <>
